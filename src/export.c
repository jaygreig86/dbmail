...
static int do_export(char *user, char *base_mailbox, char *basedir, char *outfile, char *search, int delete_after_dump, int recursive)
{
        ...
        char *dumpfile = NULL, *mailbox = NULL, *search_mailbox = NULL, *dir = NULL, *p = NULL;
        GList *children = NULL;
        int result = 0;
        FILE *subscription = NULL;
        char subscriptionpath[255];
        char controlpath[255];
        ...
        printf("Exporting [%u] mailboxes for [%s]\n", g_list_length(children), user);

        sprintf(subscriptionpath,"/%s/%s/control/.subscriptions",basedir,user);
        sprintf(controlpath,"/%s/%s/control",basedir,user);

        if (g_mkdir_with_parents(controlpath, 0700)) {
                printf("can't create directory [%s]\n", controlpath);
                result = -1;
                goto cleanup;
        }

        subscription = fopen(subscriptionpath,"a");
        ...
        while (children) {
                ...
                if (! db_get_mailbox_owner(mailbox_idnr, &owner_idnr)) {
                        printf("Error checking mailbox ownership");
                        goto cleanup;
                }
                if (owner_idnr == user_idnr) {
                        if (basedir) {
                                /* Prepare the directory */
                                p = replace_char(mailbox, '/', ".");
                                if (!strcmp(p,"INBOX")){
                                        dumpfile = g_strdup_printf("%s/%s/inbox", basedir, user);
                                        fputs("inbox\n",subscription);
                                }
                                else {
                                        dumpfile = g_strdup_printf("%s/%s/.%s", basedir, user, p);
                                        fputs(p,subscription);
                                        fputs("\n",subscription);
                                }
